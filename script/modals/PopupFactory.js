...
export class PopupFactory {
  static createModal(modalType, options = {}) {
    ...
    return {
      template: modalConfig.getTemplate(options),
      background: modalConfig.getBackground(),
      onInit: (element) => {
        if (typeof modalConfig.initForm === 'function') {
          modalConfig.initForm(element, null, options);
        }
      }
    };
  }
  ...
  static getModalConfig(type) {
    switch (type) {
      case 'order-car':      return OrderCarTemplate;
      case 'leasing':        return OrderForLeasingModal;
      case 'order-selected-car': return OrderSelectedCar;
      case 'trade-in':       return OrderTradeIn;
